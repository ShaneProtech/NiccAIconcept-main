        ...
        if self.use_supabase and self.supabase:
            ...
            if make_table:
                ...
                try:
                    ...
                    # This is more efficient than retrieving all records and filtering
                    if year_clean:
                        print(f"DEBUG: Attempting to filter by year {year_clean} in the query")
                        # Try with exact year first
                        year_query = self.supabase.table(make_table).select("*").eq("year", year_clean)
                        ...
                        if not year_response.data or len(year_response.data) == 0:
                            print(f"DEBUG: No exact year match, trying with decimal format {year_clean}.0")
                            year_query = self.supabase.table(make_table).select("*").eq("year", f"{year_clean}.0")
                            year_response = year_query.execute()
                        
                        # If we found results with year filtering
                        if year_response.data and len(year_response.data) > 0:
                            print(f"DEBUG: Found {len(year_response.data)} results with year filter {year_clean}")
                            response = year_response
                        else:
                            # If no results with year filtering, get all records and filter manually
                            print(f"DEBUG: No results with year filter, retrieving all records for manual filtering")
                            query = self.supabase.table(make_table).select("*").limit(100)
                    ...
                    if response.data:
                        print(f"DEBUG: Found {len(response.data)} initial results in {make_table} table")
                        ...
                        for item in response.data:
                            ...
                            if model_match:
                                ...
                                if year_clean:
                                    ...
                                    # Add to appropriate list based on exact match
                                    if year_clean == item_year:
                                        # Exact year match
                                        exact_year_results.append(item)
                                    else:
                                        # Not an exact match but keep for fallback
                                        filtered_results.append(item)
                                else:
                                    # No year specified, add all matches
                                    filtered_results.append(item)
                        
                        # First check if we have exact year matches
                        if year_clean and len(exact_year_results) > 0:
                            print(f"DEBUG: Found {len(exact_year_results)} exact year matches for {year_clean}")
                            return {"found": True, "data": exact_year_results, "is_multiple": len(exact_year_results) > 1}
                        
                        # If no exact year matches but we have model matches
                        elif len(filtered_results) > 0:
                            print(f"DEBUG: Found {len(filtered_results)} model matches, but no exact year matches")
                            # Only return these if no year was specified or as a fallback
                            if not year_clean:
                                return {"found": True, "data": filtered_results, "is_multiple": len(filtered_results) > 1}
                            else:
                                # Only use inexact year matches if explicitly requested or as a last resort
                                return {"found": True, "data": filtered_results, "is_multiple": len(filtered_results) > 1, 
                                        "note": f"No exact match for {year_clean} {make} {model}, showing closest available data"}
                        else:
                            print(f"DEBUG: No matching results found in {make_table} table after filtering")
                ...
                try:
                    ...
                    if year_clean:
                        ...
                        if year_response.data and len(year_response.data) > 0:
                            ...
                            # If no results with year filtering, get all records and filter manually
                            query = self.supabase.table(table).select("*").limit(100)
                            response = query.execute()
                    else:
                        # No year provided, get all records
                        query = self.supabase.table(table).select("*").limit(100)
                    ...
                    if response.data:
                        ...
                        # First check if we have exact year matches
                        if year_clean and len(exact_year_results) > 0:
                            print(f"DEBUG: Found {len(exact_year_results)} exact year matches in {table} table")
                            return {"found": True, "data": exact_year_results, "is_multiple": len(exact_year_results) > 1}
                        
                        # Fall back to other matches if necessary
                        elif len(other_results) > 0:
                            print(f"DEBUG: Found {len(other_results)} model matches in {table} table, but no exact year matches")
                            # Only return non-exact matches for vehicles if no year was specified
                            if not year_clean:
                                return {"found": True, "data": other_results, "is_multiple": len(other_results) > 1}
                            else:
